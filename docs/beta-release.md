...
1. Commit the reviewed source and tag the known-good release; the repository currently needs its first commit and remote. Run `npm run check`, `npm run build`, SQL tests, and browser tests against a preview with `AUTOMATED_TEST_MODE=true`. Never put real provider calls into CI.
