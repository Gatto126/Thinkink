...
## Deferred automatic refresh design

The following provider clocks and durable job scheduling remain future work. TopicRoom connections, authenticated presence and discussion recovery are implemented separately; see [live discussion](realtime.md).
...
Local Supabase accounts, invitation-gated test registration, cookie sessions, curated avatar selection, fixed usernames/emails and password-confirmed account deletion are implemented. Topic/comment schema establishes anonymous retention after deletion. The persistent catalogue portion of milestone 2 is implemented: public exact-title lookup, authenticated explicit creation, canonical deduplication, topic retrieval and Latest lists. Initial Serper news and free OpenRouter overview preparation now persists sources and input snapshots, with atomic budgets, bounded fallback and explicit recovery. See [provider limits](content-providers.md). Most visited, comment composition, replies, likes and favorites are implemented. Topic rooms deliver live discussion updates with snapshot recovery; periodic provider updates remain pending. Owner deletion from account settings and a role-protected topic/comment moderation dashboard are implemented; deleting a topic also deletes its comments atomically. Local automatic email confirmation is restricted to loopback app/database URLs; the HTTPS beta retains invitation-only immediate signup without email confirmation. See [local setup](local-setup.md) for reproducible commands and the exact test boundary.
